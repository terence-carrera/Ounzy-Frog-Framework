...
class Scaffold extends Command
{
    ...
    public function handle(array $arguments = []): int
    {
        ...
        // Service (inside framework/App/Services)
        $serviceDir = $srcDir . '/App/Services';
    ...
    HTML;
        ...
        // Controller (inside framework/App/Controllers)
        $controllerDir = $srcDir . '/App/Controllers';
    ...
    PHP;
        ...
        $routeLine = "Route::get('/{$view}', [\\Frog\\App\\Controllers\\{$controller}::class, 'index'])->name('{$view}');";
        if (is_file($routesFile) && is_writable($routesFile)) {
            $routesContent = file_get_contents($routesFile) ?: '';
            if (!str_contains($routesContent, $routeLine)) {
                // Ensure Route facade imported
                if (!str_contains($routesContent, 'use Frog\\Infrastructure\\Routing\\Route;')) {
                    ...
                    if ($lastUseIndex >= 0) {
                        array_splice($routesLines, $lastUseIndex + 1, 0, 'use Frog\\Infrastructure\\Routing\\Route;');
                        $routesContent = implode("\n", $routesLines);
                    } else {
                        // Fallback: add after opening tag
                        $routesContent = preg_replace('/<\?php/', "<?php\nuse Frog\\Infrastructure\\Routing\\Route;", $routesContent, 1);
                    }
                }
                // Append route at end
